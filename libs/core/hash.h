...
size_t hashBytes(void const * ptr, size_t count);

size_t hashStr(char const src[]);

// calculates hash up to trailing null or maxlen, whichever comes first
size_t hashStr(char const src[], size_t maxlen);
...
uint32_t hash_crc32c(uint32_t crc, void const * ptr, size_t count);

// Helper to calculate a new CRC
inline uint32_t hash_crc32c(void const * ptr, size_t count) {
