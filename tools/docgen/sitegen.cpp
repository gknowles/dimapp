...
enum ContentAction {
    kContentActionInvalid,
    kContentActionMarkup,
    kContentActionInterpolate,
};
...
//===========================================================================
static void addFileHeader(CharBuf * out, const Path & fname) {
...
//===========================================================================
static void addHtmlHeader(CharBuf * out, const Path & fname) {
    out->append("<!DOCTYPE html>\n<!--\n");
    addFileHeader(out, fname);
    out->append("-->\n");
}

//===========================================================================
static void addAttrValueWithIntegrity(
    IXBuilder * out,
    string_view relPath,
    string_view integrity,
    string_view fallbackPath
) {
    if (fallbackPath.empty()) {
        out->text(relPath).endAttr();
    } else {
        out->text(fallbackPath).endAttr();
        out->attr("crossorigin", "anonymous");
        if (!integrity.empty()) {
            out->attr("integrity", integrity);
        }
    }
    if (!fallbackPath.empty()) {
        out->startAttr("onerror")
            .text("this.onerror=null; delete this.crossorigin; "
                "delete this.integrity; this.href = '")
            .text(relPath)
            .text("';")
            .endAttr();
    }
...
//===========================================================================
static void addScriptSrc(
    IXBuilder * out,
    string_view relPath,
    string_view integrity = {},
    string_view fallbackPath = {}
) {
    out->start("script").startAttr("src");
    addAttrValueWithIntegrity(out, relPath, integrity, fallbackPath);
    out->end();
}
...
static void addCssLink(
    ...
    out->start("link").attr("rel", "stylesheet").startAttr("href");
    addAttrValueWithIntegrity(out, relPath, integrity, fallbackPath);
    out->end();
}

//===========================================================================
static void addFaviconLink(
    IXBuilder * out,
    const Path & fname,
    const Config & cfg
) {
...
//===========================================================================
static void addFontAwesomeHead(IXBuilder * out, const Page & page) {
    addCssLink(
        out,
        page.urlRoot / "vendor" / "font-awesome@4.7.0"
...
document.addEventListener('DOMContentLoaded', (event) => {
  document.querySelectorAll('pre code').forEach((block) => {
    ...
    block.classList.add(lang);
        ...
        .end();
    addCssLink(out, page.urlRoot / "vendor" / "prismjs@1.30.0" / "prism.css");
    addScriptSrc(out, page.urlRoot / "vendor" / "prismjs@1.30.0" / "prism.js");
}

//===========================================================================
static void addBootstrapHead(IXBuilder * out, const Page & page) {
    auto & bld = *out;
    bld.start("meta")
        .attr("name", "viewport")
    ...
    addCssLink(
        out,
        page.urlRoot / "vendor" / "bootstrap@4.6.2" / "bootstrap.min.css",
"sha384-xOolHFLEh07PJGoPkLv1IbcEPTNtaed2xpHsD9ESMhqIYd0nLMwNLD69Npy4HI+N",
        "https://cdn.jsdelivr.net/npm/bootstrap@4.6.2/dist/"
            "css/bootstrap.min.css"
    );
}

//===========================================================================
static void addBootstrapBody(IXBuilder * out, const Page & page) {
    addScriptSrc(out,
        page.urlRoot / "vendor" / "jquery@3.5.1" / "jquery.slim.min.js",
"sha384-DfXdz2htPH0lsSSs5nCTpuj/zy4C+OGpamoFVy38MVBnE+IbbVYUew+OrCXaRkfj",
        "https://cdn.jsdelivr.net/npm/jquery@3.5.1/dist/jquery.slim.min.js"
    );
    addScriptSrc(out,
        page.urlRoot / "vendor" / "popper.js@1.16.1" / "popper.min.js",
"sha384-9/reFTGAW83EW2RDu2S0VKaIzap3H66lZH81PoYlFhbGU+6BZp6G7niu735Sk7lN",
"https://cdn.jsdelivr.net/npm/popper.js@1.16.1/dist/umd/popper.min.js"
    );
    addScriptSrc(out,
        page.urlRoot / "vendor" / "bootstrap@4.6.2" / "bootstrap.min.js",
...
//===========================================================================
static string codeToHtmlFragment(const Page & page, const string & content) {
    CharBuf buf;
    XBuilder bld(&buf);
    bld.start("h1")
        ...
        .text(page.name)
        .end();
    bld.startDoc().elem("br");
    bld.startDoc().start("pre")
        .attr("lang", page.typeInfo->preLang)
...
static CharBuf processPageContent(
    ...
    addFaviconLink(&bld, fname, *cfg);
    addPrismJsHead(&bld, info->page);
    addBootstrapHead(&bld, info->page);
    addFontAwesomeHead(&bld, info->page);
    for (auto&& f : {
        "code/docgen.css",
        "code/asciidoc.css",
        "code/prism.css"
    }) {
        addCssLink(&bld, info->page.urlRoot / f);
    }
    bld.elem("script", 1 + R"(
document.addEventListener('DOMContentLoaded', (event) => {
  ...
  */
    ...
    addBootstrapBody(&bld, info->page);
    bld.end() // </body>
        .end(); // </html>
    return html;
}
...
static void genPage(GenPageInfo * info, unsigned phase = 0) {
    ...
    if (phase == what++) {
        ...
        if (!info->page.patch.empty()
            || info->page.typeInfo->process == PageProcess::kMarkupToHtml
        ) {
    ...
    if (phase == what++) {
        if (info->page.typeInfo->process == PageProcess::kMarkupToHtml) {
        ...
        assert(info->page.typeInfo->process == PageProcess::kCodeToHtml);
        phase = what;
    }
    if (phase == what++) {
        // Update HTML fragment, embed into HTML page, and add to site output
        // files.
        if (info->page.typeInfo->process == PageProcess::kCodeToHtml) {
            // Convert code block into HTML fragment
            auto html = codeToHtmlFragment(info->page, info->content);
            info->content = move(html);
        }
...
static bool genEmbeddedStatics(Config * out) {
    ...
    for (auto&& f : files) {
        Path fn = Path(f.first).relative("/");
        CharBuf buf;
        if (fn.view().starts_with("vendor/")) {
            buf.assign(f.second.content);
        } else {
            auto content = interpContent(f.second.content);
            buf.assign(move(content));
        }
        if (!addOutput(out, fn, move(buf)))
            return false;
    }
    return true;
}
