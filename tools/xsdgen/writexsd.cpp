...
struct XsdInfo {
    unordered_set<void const *> processed;
    unordered_map<string_view, unsigned> nameCounts;
    unordered_map<Element const *, string> elemNames;
    unordered_map<Attr const *, string> attrNames;
};
...
//===========================================================================
static void writeDesc(IXBuilder * out, string const & desc) {
   if (!desc.empty())
...
static string typeName(
    ...
    unordered_map<T const *, string> * names;
...
//===========================================================================
static void writeAttr(IXBuilder * out, XsdInfo * info, Attr const & attr) {
    out->start("attribute")
        .attr("name", attr.name);
    if (attr.require)
...
//===========================================================================
static void writeElem(IXBuilder * out, XsdInfo * info, Element const & elem) {
    out->start("element")
        .attr("name", elem.name)
        .startAttr("type")
...
//===========================================================================
bool writeXsd(CharBuf * out, Schema const & schema) {
    out->append("<!--\n")
