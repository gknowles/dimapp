...
static void writeCppfileStart(
    ostream & os, const set<Element> & rules, const string & classPrefix) {
    time_t now = time(nullptr);
    tm tm;
    localtime_s(&tm, &now);
    os << 1 + R"(
// abnfparse.cpp
// Autogenerated by pargen at )"
...
class )"
       << classPrefix << R"(Parser {
public:
    )" << classPrefix
       << "Parser (I" << classPrefix
...
void writeParser(ostream & hfile, ostream & cppfile, const set<Element> & src) {
    const string & root = getOption(src, kOptionRoot);
    ...
    const string & prefix = getOption(src, kOptionApiPrefix);
    writeHeaderfile(hfile, rules, prefix);
    writeCppfileStart(cppfile, rules, prefix);

    unordered_set<State> states;
    buildStateTree(&states, rules, kOptionRoot, s_buildStateTree);
    writeFunction(
        cppfile, nullptr, rules, states, prefix, s_writeStatePositions);
